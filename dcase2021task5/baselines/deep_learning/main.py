...
def init_wandb():
    ...
    wandb.init(name="4CNN+KL_Loss",
               project='Deep_Learning_Final',
               notes='Prototypical variant',
               save_code=False)
    
    code = wandb.Artifact('project-source', type='code')
    for path in glob('/home/gqw98/dcaseTask5/baselines/deep_learning/*.py', recursive=True):
        print(path)
        code.add_file(path)
...
def train_protonet(model,train_loader,valid_loader,conf,num_batches_tr,num_batches_vd):
    ...
    for epoch in range(num_epochs):
        ...
        # wandb.log({
        #     "Epoch": epoch,
        #     "Tra/ Loss": avg_loss_tr,
        #     "Tra/ acc": avg_acc_tr,
        #     "Val/ Loss": avg_loss_vd,
        #     "Val/ acc": avg_acc_vd,
        #     })
...
def main(conf : DictConfig):
    ...
    if conf.set.train:
        ...
        best_acc,model,best_state = train_protonet(encoder,train_loader,valid_loader,conf,num_batches_tr,num_batches_vd)
    ...
    if conf.set.eval:
        ...
        csv_path = os.path.join(conf.path.root_dir,'Eval_out_1.csv')
